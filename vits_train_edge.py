...
def run(rank, n_gpus, hps):
    ...
    #dist.init_process_group(backend=backend_str, init_method="env://", world_size=n_gpus, rank=rank)
    ...
    try:
        ...
        global_step = 0
    """
    utils.load_model('./models/G_BZN.pth', net_g)
    utils.load_model('./models/D_BZN.pth', net_d)
    epoch_str = 1
    global_step = 0
    ...
    for batch_idx, (x, x_lengths, bert, spec, spec_lengths, y, y_lengths) in enumerate(loader):
        ...
        if rank == 0:
            ...
            if amel <=18.5 and adur <= 0.06 and epoch >=100:
                utils.save_model(net_g,'./temp/gen/vits.pth')
                sys.exit(1)
