...
import vits_core.modules as modules
import vits_core.attentions as attentions
import vits_core.monotonic_align as monotonic_align

from torch.nn import Conv1d, ConvTranspose1d, AvgPool1d, Conv2d
from torch.nn.utils import weight_norm, remove_weight_norm, spectral_norm
from vits_core.commons import init_weights, get_padding
...
class DurationPredictor_temp(nn.Module):
    def __init__(
        self, in_channels, filter_channels, kernel_size, p_dropout, gin_channels=0
    ):
        super().__init__()
        ...
        self.norm_init = modules.LayerNorm(filter_channels)
        self.dps = nn.ModuleList()
        for i in range(self.n_dp_layers-1):
                self.dps.append(
                    nn.Conv1d(
                    filter_channels, filter_channels, kernel_size, padding=kernel_size // 2
                )
            )
        self.dp_norms = nn.ModuleList()
        for i in range(self.n_dp_layers-1):
            self.dp_norms.append(
    ...
    def forward(self, x, x_mask, g=None):
        ...
        if g is not None:
            g = torch.detach(g)
            x = x + self.cond(g)
        x = self.conv_init(x * x_mask)
        x = torch.relu(x)
        x = self.norm_init(x)
        x = self.drop(x)
        for dps,dp_norm in zip(self.dps,self.dp_norms):
            x = dps(x * x_mask)
            x = torch.relu(x)
            x = dp_norm(x)
            x = self.drop(x)
...
class SynthesizerEval_temp(nn.Module):
    """
    Synthesizer for Training
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.dp = DurationPredictor_temp(
            hidden_channels, 256, 3, 0.5, gin_channels=gin_channels
        )
        #if n_speakers > 1:
        #    self.emb_g = nn.Embedding(n_speakers, gin_channels)
...
class SynthesizerTrn_temp(nn.Module):
    """
    Synthesizer for Training
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        print('gin_channels=',gin_channels)
        self.n_vocab = n_vocab
        self.spec_channels = spec_channels
        self.inter_channels = inter_channels
        self.hidden_channels = hidden_channels
...
class TextEncoder_no_bert(nn.Module):
    def __init__(
        self,
        n_vocab,
        out_channels,
...
class SynthesizerTrn_no_bert(nn.Module):
    """
    Synthesizer for Training
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.enc_p = TextEncoder_no_bert(
            n_vocab,
            inter_channels,
            hidden_channels,
            filter_channels,
...
class SynthesizerEval_no_bert(nn.Module):
    """
    Synthesizer for Training
    """
...
class SynthesizerTrn_no_bert_sdp(nn.Module):
    """
    Synthesizer for Training
    """
...
class TextEncoder_multilingual(nn.Module):
    def __init__(
        self,
        n_vocab,
        out_channels,
        ...
        self.proj = nn.Conv1d(hidden_channels, out_channels * 2, 1)
        self.emb_lang = nn.Linear(gin_channels, hidden_channels)

    def forward(self, x, x_lengths,lang=None):
        x = self.emb(x) * math.sqrt(self.hidden_channels)  # [b, t, h]
        b = self.emb_lang(lang)
        x = x + b
        x = torch.transpose(x, 1, -1)  # [b, h, t]
        x_mask = torch.unsqueeze(commons.sequence_mask(x_lengths, x.size(2)), 1).to(
            x.dtype
...
class DurationPredictor_multilingual(nn.Module):
    def __init__(
        self, in_channels, filter_channels, kernel_size, p_dropout, gin_channels=0
    ):
        super().__init__()
        ...
        if gin_channels != 0:
            self.cond = nn.Conv1d(gin_channels, in_channels, 1)
            self.cond2 = nn.Conv1d(gin_channels, in_channels, 1)

    def forward(self, x, x_mask, g=None,b_lang=None):        
        x = torch.detach(x)
        if g is not None:
            g = torch.detach(g)
            x = x + self.cond(g)
        if b_lang is not None:
            b_lang = torch.detach(b_lang)
            x = x + self.cond2(b_lang)
...
class SynthesizerTrn_multilingual(nn.Module):
    """
    Synthesizer for Training
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.enc_p = TextEncoder_multilingual(
            n_vocab,
            inter_channels,
            hidden_channels,
            filter_channels,
        ...
        self.dp = DurationPredictor_multilingual(
            hidden_channels, 256, 3, 0.5, gin_channels=gin_channels
        )
        #if n_speakers > 1:
        #    self.emb_g = nn.Embedding(n_speakers, gin_channels)
        #if n_languages > 1:
        #    self.emb_l = nn.Embedding(n_languages, gin_channels)
        self.emb_l = nn.Embedding(n_languages, gin_channels)

    def forward(self, x, x_lengths, y, y_lengths, sid=None, lid=None,lid_lengths=None,b_lid=None):
        #if self.n_speakers > 0:
        #    g = self.emb_g(sid).unsqueeze(-1)  # [b, h, 1]
        #else:
        #    g = None
        #print('=========',lid.shape)
        lang = self.emb_l(lid)  # [b,t, h]
        b_lang = self.emb_l(b_lid).unsqueeze(-1)  # [b, h, 1]
        g = sid.unsqueeze(-1)  # [b, h, 1]
        x, m_p, logs_p, x_mask = self.enc_p(x, x_lengths,lang=lang)
        z, m_q, logs_q, y_mask = self.enc_q(y, y_lengths, g=g)
        z_p = self.flow(z, y_mask, g=g)
        ...
        w = attn.sum(2)
        logw_ = torch.log(w + 1e-6) * x_mask
        logw = self.dp(x, x_mask, g=g,b_lang=b_lang)
        l_length = torch.sum((logw - logw_) ** 2, [1, 2]) / torch.sum(x_mask)  # for averaging
        # expand prior
        m_p = torch.matmul(attn.squeeze(1), m_p.transpose(1, 2)).transpose(1, 2)
        logs_p = torch.matmul(attn.squeeze(1), logs_p.transpose(1, 2)).transpose(1, 2)
    ...
    def infer(self, x, x_lengths, sid=None,lid=None,lid_lengths=None,b_lid=None,noise_scale=1, length_scale=1 ,max_len=None):
        lang = self.emb_l(lid)  # [b, t ,h]
        b_lang = self.emb_l(b_lid).unsqueeze(-1)  # [b, h, 1]
        x, m_p, logs_p, x_mask = self.enc_p(x, x_lengths,lang=lang)
        #if self.n_speakers > 0:
